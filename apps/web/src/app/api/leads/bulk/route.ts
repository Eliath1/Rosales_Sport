...
import { createLead } from "@rs/shared/services/leadService";
import { bulkFormLeadSchema } from "@rs/shared/validators/schemas";

export async function POST(request: Request) {
  const body = await request.json();
  const parsed = bulkFormLeadSchema.safeParse(body);
  if (!parsed.success) {
